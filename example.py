...
def example_single_observation():
    ...
    print(f"Round Trip: {observation.round_trip_time}s ({observation.round_trip_time/60:.1f} min)")
    ...
    print(f"Distance: {hive.distance_from_observer:.0f}m ({hive.distance_from_observer/1000:.2f}km)")
...
def example_multiple_observations():
    ...
    observations = [
        Observation(
            ...
            bearing=45.0, speed=7.0, round_trip_time=300,
            notes="Observation Point 1 - Eiffel Tower"
        ),
        Observation(
            latitude=48.8606, longitude=2.2900,  # ~400m NW of point 1
            bearing=90.0, speed=7.0, round_trip_time=280,
            notes="Observation Point 2 - Trocadéro"
        ),
        Observation(
            latitude=48.8560, longitude=2.2970,  # ~300m SE of point 1
            bearing=30.0, speed=7.0, round_trip_time=310,
    ...
    individual_hives = [
        calculator.calculate_from_single_observation(obs)
    ...
    map_file = visualizer.create_map(
        ...
        hive_locations=[hive],
        output_file="example_triangulation.html"
    )
    print(f"\n✓ Map saved to: {map_file}")
...
def example_different_speeds():
    ...
    for speed in speeds:
        ...
        print(f"  {speed:.1f} m/s | {obs.estimated_distance:4.0f}m   | "
